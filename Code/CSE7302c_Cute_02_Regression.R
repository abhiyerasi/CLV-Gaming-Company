...
hopmonkdata=read.csv("hopmonkdatawitgoodvalue.csv",header=T)
...
target=subset(hopmonkdata,select = c(TotalRevenueGenerated))
cathopmonk=hopmonkdata[sapply(hopmonkdata,is.factor)]
numerichopmonk=hopmonkdata[sapply(hopmonkdata,is.numeric)]
numerichopmonk=scale(subset(numerichopmonk,select = -c(TotalRevenueGenerated)))

hopmonkdata=data.frame(numerichopmonk,cathopmonk,target)

#hopmonkdata$TotalRevenueGenerated=sinh(hopmonkdata$TotalRevenueGenerated)
...
inTrain <- createDataPartition(hopmonkdata$TotalRevenueGenerated, p = .7, list = FALSE)
...
LinReg1 =lm(formula =TotalRevenueGenerated~., data = Train)
...
train_error=regr.eval(Train$TotalRevenueGenerated, LinReg1$fitted.values) 
train_error_df=data.frame(train_error)

#Error verification on test data
test=subset(Test,select = -c(TotalRevenueGenerated))
...
# BUILD LINEAR REGRESSION MODEL 
AIC_LinReg =lm(formula =TotalRevenueGenerated ~ MaxChildAge + MinChildAge + UNITS + UNITS360 + 
                 TenureDays + FrequencyLF + FrequencyApp + FreqGamePlay + 
                 Frequencylf30 + FrequencyApp30 + RecencyAPP + Recencylf + 
                 Recencylf30 + Recencylf360 + maxRecencyCum + minRecencyCum + 
                 No_of_Childrens + FrequencyApp360 + FreqGamePlay360 + Revenue180 + 
                 Revenue360 + Country + FavoriteChannel + FavoriteChannel30 + 
                 FavoriteChannel360 + FavoriteGame360 + Strength_of_FavoriteGame,data = AIC_Train)
...
#Error verification on train data
AIC_train_error=regr.eval(AIC_Train$TotalRevenueGenerated, AIC_LinReg$fitted.values) 
AIC_train_error_df=data.frame(AIC_train_error)

#Error verification on test data
AIC_test=subset(AIC_Test,select = -c(TotalRevenueGenerated))
AIC_target=AIC_Test$TotalRevenueGenerated
...
xfactors <- model.matrix( ~ Country+FavoriteSource+FavoriteChannel+
                            FavoriteGame+FavoriteSource30+FavoriteChannel30+FavoriteSource180+
                            FavoriteChannel180+FavoriteGame30+FavoriteGame180,data = hopmonkdata)[,-1]

numeric_ridge_lasso=hopmonkdata[sapply(hopmonkdata,is.numeric)]
numeric_ridge_lasso=subset(numeric_ridge_lasso,select=-c(TotalRevenueGenerated))
lasso_target=subset(hopmonkdata,select=c(TotalRevenueGenerated))
...
#Target Varaible
ytrain=lasso_target$TotalRevenueGenerated[inTrain]
ytest = lasso_target$TotalRevenueGenerated[-inTrain]
...
#Target Varaible
ytrain_ridge=lasso_target$TotalRevenueGenerated[inTrain]
ytest_ridge = lasso_target$TotalRevenueGenerated[-inTrain]
...
x=Train[,-36]
y=Train$TotalRevenueGenerated
x_test=Test[,-36]
y_test=Test$TotalRevenueGenerated
...
library(DMwR)
regr.eval(kfold_test,y_test)
regr.eval(kfold_train,y)
